...
cpdefine("inline:com-zipwhip-widget-texterator", ['Three', "chilipeppr_ready",  /* other dependencies here */ ], function() {
    return {
        ...
        init: function(opts) {
            ...
            // turn on 3d viewer initially cuz it was making people think stuff was broken
            // instead we turn it off after we render a 3d file so it uses less cpu
            setTimeout(function() {
        ...
        //navigator: null,
        stopAfterCanPlay: false, // on init, this will stop vid track
        /**
            ...
            navigator.getUserMedia({video: true, audio: false}, 
                function(localMediaStream) {
                    ...
                    videoEl.onloadedmetadata = function(e) {
                      ...
                      console.log("video. got onloadedmetadata. e:", e);
                    };
                    
                    that.stopAfterCanPlay = true;
                    
                    videoEl.addEventListener('canplay', that.camOnCanPlay.bind(that));
                    /*
        ...
        isActiveOnCupPickupSnapPhoto: false, // debounce all other calls until done
        camOnCupPickupSnapPhotoUpdateScreensaverAndSendMms: function(phone) {
            
            // debounce
            // cuz we can't get called at the same time as the cam snap process is occurring
            // cuz it's long and async
            if (this.isActiveOnCupPickupSnapPhoto) {
                console.error("cam. got asked to take a photo while another was in process. skipping.");
                return;
            }
            
            this.isActiveOnCupPickupSnapPhoto = true;
            ...
            this.camPostPhotoCallback = function(dataUrl) {
                ...
                // mark that we're done taking the photo so another one can get taken
                that.isActiveOnCupPickupSnapPhoto = false;
        ...
        camOnCanPlay: function(ev) {
            ...
            if (this.stopAfterCanPlay) {
                this.camStop();
                this.stopAfterCanPlay = false;
                return;
            }
        ...
        camImmediatelySnapPhoto: function() {
            ...
            var logoWidth = 698; //logoEl.width();
            var logoHeight = 274;
            console.log("video. logoWidth:", logoWidth, "logoEl:", logoEl);
            var ratio = width / logoWidth;
            var newLogoWidth = parseInt(logoWidth * ratio); // parseInt(logoEl.width() * ratio);
            var newLogoHeight = parseInt(logoHeight * ratio); // parseInt(logoEl.height() * ratio);
        ...
        sendMmsMessage: function(imageDataUrl, toPhone, msg) {
            ...
            // this gets called with the active sessionkey
            var onGotSessionKey = function(payload) {
            ...
            // get sessionkey from Zipwhip Recv Text widget
            var request = {
                callback: onGotSessionKey
            }
            chilipeppr.publish("/com-chilipeppr-widget-recvtext/getSessionKey", request);
            
            
        },
        uploadImage: function() {
            // WebKitFormBoundaryOBpwlhFMzHEflePF
            // https://landline.zipwhip.com/hostedContent/save/verbose/?responseType=text&session=534bcea8-2595-4893-a4ce-98831b67d956:303273303    
            // {"response":[{"fileName":"beerbot.png","fileSizeBytes":289037,"mimeType":"image/png","storageKey":"3a844b40-41b1-448a-9da7-714a07971018"}],"success":true}
            // https://landline.zipwhip.com/hostedContent/get/?storageKey=3a844b40-41b1-448a-9da7-714a07971018&session=534bcea8-2595-4893-a4ce-98831b67d956:303273303
            // sending....
            // WebKitFormBoundarym3jNTtZY9atv56SD
            // https://landline.zipwhip.com/messaging/send?session=534bcea8-2595-4893-a4ce-98831b67d956:303273303&to=+13134147502&body=&contentTypeOverride=text%2Fhtml
            // {"response":{"id":712412071737958400,"status":"queued"},"success":true}
...
`,
        ...
        onSerialIncomingLine: function(line) {
            ...
            if (line.match(/^{/)) {
                ...
                // {"e":{,"etype":1"slots":[1,2,0,0,0,0,0,0],"index":65}}
                line = line.replace(/{,"et/, '{"et');
                line = line.replace(/0"slots"/, '0,"slots"');
                line = line.replace(/1"slots"/, '1,"slots"');
                line = line.replace(/2"slots"/, '2,"slots"');
                line = line.replace(/3"slots"/, '3,"slots"');
        ...
        statusUpdated: function(sr) {
            ...
            for (var key in sr) {
                if (key == "index") {
                    ...
                    // parse slots
                    // var slotArr = sr.slots.split(/,/);
                    var slotTxtArr = [];
                    for (var i = 0; i < sr.slots.length; i++) {
                        // var slotVal = parseInt(slotArr[i]);
            ...
            // check if laser idle and if so wipe the etype event
            if ('laser_state' in sr && sr.laser_state == 1) {
                el.find('.etype').text("-");
            }
            
            // see if we saw an index change
            if ('index' in sr && sr.index != this.lastTableIndex) {
        ...
        onTableIndexChange: function(newindex) {
            ...
            // this is THE KEY EVENT for indicating that slots should move forward so we have a synced
            // view of who/what queueId and user has what cup in what slot
            // this is so we know when they pick up a cup that it's their's that was picked up
            this.advanceForwardSlots();
        ...
        advanceForwardSlots: function() {
            console.log("slotstatus. advanceForwardSlots");
            // move all the array items forward 1. if array is at end then put that at 0.
            this.slotState.unshift(this.slotState.pop());
        ...
        onSlotStatusChange: function(newslots) {
            ...
            // we need to look at the changes and see if a cup was just picked up
            for (var i = 0; i < 8; i++) {
                // if last status of slot was beer (2) or marked (3)
                // and now the new status is nocup (0)
                // then we have a cup pickup event!!!!!!
                if (this.lastSlots[i] >= 2 && newslots[i] == 0) {
                    this.onCupPickedUp(this.slotState[i], i);
                }
            }
        ...
        refreshSlotStatus: function() {
            ...
            console.log("slotstatus. refreshing");
        ...
        /**
         * We get this call when the slot status changes from beer/marked to nocup
         * We may get a bunch of these at the same time, so debounce cuz the 10 sec process
         * to snap a photo can only really happen for the first person and the rest will just
         * have to get ignored.
         */
        onCupPickedUp: function(obj, slotIndex) {
            ...
            // debounce if we are already Cup gone
            if (obj.phone.match(/Cup gone/i)) {
                console.error("cup gone already hit for this slot/user. returning.");
                return;
            }
            
            // if queueId is currently being lasered, ignore this signal cuz it's a lie
            if (obj.queueId == this.activeQueueIdLasering) {
            ...
            // now update the database to change that the status is now Cup gone.
            this.getQueue(obj.queueId, function(qrec) {
                qrec.status = "Cup gone";
                that.updateQueue(qrec, function() {
                    console.log("onCupPickedUp. updateQueue finished. can now refresh view.")
                    that.refreshQueueView();
                });
            })
        },
