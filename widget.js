...
cpdefine("inline:com-zipwhip-widget-texterator", ["chilipeppr_ready", /* other dependencies here */ ], function() {
    return {
        ...
        camTakePicture: function() {
            ...
            var width = this.videoEl.videoWidth; //context.canvas.clientWidth;
            var height = this.videoEl.videoHeight; //context.canvas.clientHeight;
            console.log("video. width:", width, ", height:", height);
            // if (width && height) {
            canvas.width = width;
            canvas.height = height + logoEl.height();
            context.drawImage(this.videoEl, 0, 0, width, height);
            
            
            //canvas.height += logoEl.height();
            // draw at bottom of image
            context.drawImage(logo, 0, height);
